...
class AssetController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($validated, $request) {
            ...
            return response()->json(
                $asset->load(['category', 'location', 'currentUser', 'barcode']),
                201
            );
        });
    }
    ...
    public function show(Asset $asset)
    {
        return response()->json(
            $asset->load([
                ...
                'userHistory.assignedBy',
                'locationHistory.fromLocation',
    ...
    public function update(Request $request, Asset $asset)
    {
        $validated = $request->validate([
            ...
            'location_id' => 'nullable|exists:locations,id',
            'current_user_id' => 'nullable|exists:users,id',
            'status' => 'sometimes|string|in:active,maintenance,storage,borrowed,disposed,lost',
            'notes' => 'nullable|string',
        ]);

        return DB::transaction(function () use ($validated, $request, $asset) {
            // Catat log status jika berubah
            if (isset($validated['status']) && $validated['status'] !== $asset->status) {
                AssetStatusLog::create([
                    'asset_id' => $asset->id,
                    'old_status' => $asset->status,
                    'new_status' => $validated['status'],
                    'notes' => 'Diubah lewat formulir edit aset',
                    'changed_by' => $request->user()->id,
                    'changed_at' => now(),
                ]);
            }

            // Catat pemindahan lokasi jika berubah
            if (array_key_exists('location_id', $validated) && $validated['location_id'] !== $asset->location_id) {
                AssetLocationHistory::create([
                    'asset_id' => $asset->id,
                    'from_location_id' => $asset->location_id,
                    'to_location_id' => $validated['location_id'],
                    'reason' => 'Diubah lewat formulir edit aset',
                    'moved_by' => $request->user()->id,
                    'moved_at' => now(),
                ]);
            }

            // Catat penugasan pengguna jika berubah
            if (array_key_exists('current_user_id', $validated) && $validated['current_user_id'] !== $asset->current_user_id) {
    ...
    public function uploadPhoto(Request $request, Asset $asset)
    {
        ...
        return response()->json(['photo_url' => $disk->url($path)]);
    }
